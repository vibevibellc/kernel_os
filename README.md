...
- QEMU now defaults to `DISPLAY_BACKEND=auto`. On macOS that resolves to `-display cocoa,show-cursor=on,zoom-to-fit=on,full-screen=off`, which is friendlier across small and large host displays. Override `DISPLAY_BACKEND`, `DISPLAY_ZOOM_TO_FIT`, `DISPLAY_SHOW_CURSOR`, or `DISPLAY_FULL_SCREEN` if you want to force different behavior.
- The kernel text monitor now prefers VGA 80x50 text and preserves that denser geometry across console clears, so the terminal window stays usable on a wider range of host window sizes. If a path drops out of text mode, it still falls back cleanly through BIOS mode `03h`.
