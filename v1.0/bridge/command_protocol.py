...
RUNTIME_STATE_PATTERN = re.compile(r"^KERNEL-RUNTIME\s+(latent|busy)\s*$", re.IGNORECASE)
...
def match_runtime_state(line: str) -> dict | None:
    match = RUNTIME_STATE_PATTERN.match(line.strip())
    if not match:
        return None
    return {
