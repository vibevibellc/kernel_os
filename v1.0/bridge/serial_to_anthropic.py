...
from command_protocol import match_pending_observation, match_runtime_state
from git_sync_debounce import GitSyncDebouncer
from live_patch_persistence import persist_stage2_patch, select_patch_verification
...
def record_pending_observation(
    ...
    recent_peeks_by_session: dict[str, deque[dict]],
    line: str,
    ...
    if payload.get("kind") == "peek":
        ...
        if session_id:
            recent_peeks_by_session.setdefault(session_id, deque(maxlen=16)).append(
                {
    ...
    if payload.get("kind") == "patch":
        try:
            ...
            verification = select_patch_verification(
                payload["origin"],
                list(recent_peeks_by_session.get(session_id, ())),
            )
            changed_paths = persist_stage2_patch(
                payload["origin"],
                payload["observation"],
...
def record_runtime_state(webhook: str, line: str) -> bool:
    payload = match_runtime_state(line)
    if payload is None:
        return False
    data = forward_request(webhook, "/host", {"action": "kernel-runtime", **payload})
    print(f"Kernel runtime: {sanitize_line(data.get('message', line))}")
    return True
            ...
            try:
                with sock:
                    ...
                    for raw_line in reader:
                        ...
                        if not line.startswith(REQUEST_PREFIX):
                            try:
                                if record_runtime_state(args.webhook, line):
                                    continue
